...
#else
/* MULxPOW.
 * Loop version created for SBT simulator
 * Input V: an 8-bit input.
 * Input i: a positive integer.
 * Input c: an 8-bit input.
 * Output : an 8-bit output.
...
/*
 *  Looks like we don't need the upper part anymore :)
 *
 * */
 ...
 * See section 3.3.1.
 *
 * reference implementation of SNOW 3G S-box 1. Optimized is given by a LUT.
u32 S1(u32 w)
{
...
u32 S1(u32 w) {
    u32 sub =   s1_t0[(w & 0x000000FF)] ^ 
                s1_t1[(w & 0x0000FF00) >>  8] ^
                s1_t2[(w & 0x00FF0000) >> 16] ^
                s1_t3[(w & 0xFF000000) >> 24];
    return sub;
}
...
u32 S2(u32 w) {
    u32 sub =   s2_t0[(w & 0x000000FF)] ^ 
                s2_t1[(w & 0x0000FF00) >>  8] ^
                s2_t2[(w & 0x00FF0000) >> 16] ^
                s2_t3[(w & 0xFF000000) >> 24];
    return sub;
}
...
void ClockLFSRInitializationMode(u32 F)
{
    u32 v = (((LFSR_S0 << 8) & 0xffffff00) ^
            (mul_alpha[(u8)((LFSR_S0>>24) & 0xff)]) ^
            (LFSR_S2) ^
            ((LFSR_S11 >> 8) & 0x00ffffff) ^
            (div_alpha[(u8)((LFSR_S11) & 0xff)]) ^
            (F)
            );
    LFSR_S0 = LFSR_S1;
    LFSR_S1 = LFSR_S2;
